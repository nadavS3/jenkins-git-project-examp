...
pipeline
{
	...
	environment {
		...
		PM2_NAME = 'digital-orientation'
		DOMAIN_NAME = 'digital-orientation.carmel6000.com'
		NODE_ENV = ''
		CI=false
		PORT=8252
		CARMEL_SUBDOMAIN = 'digital-orientation'
    ...
    stages
    {
		...
		sh '''
		  for client in client*; do
		   ...
		   cd ..
                '''
//added command for digital-Orientation!!! , not needed nor will work in other projects, creates symbolic link to uploads because our client-admin 
//has different domain so our server doesent serve him the uploads directory. we need to redirect the requests to the actual uploads in client-admin for that 
		sh '''
		cd client-admin/build
		   ln -s ../../uploads/* .
		   cd ../../
		'''
...
do
	...
	echo ""
		...
		try {
				...
				sh "export NODE_ENV=production; cd server; pm2 start dist/main.js --name ${PM2_name}"
			}
		}
		}
       }
